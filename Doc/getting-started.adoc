...
NOTE: To perform any of these actions, the user must be previously authenticated.

==== Mute a user

In order to mute a user, you have to call <<K2_MuteUserAsync>> with the corresponding UserID and a callback given the asynchronous nature of the function
...
To perform the inverse operation, unmute a user, there is a similar function named <<K2_UnmuteUserAsync>> with the corresponding UserID and a callback given the asynchronous nature of the function
...
This method returns a users list previously muted by an authenticated user. This function name is <<K2_GetMutedUsersAsync>>>
