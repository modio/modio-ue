...
doWrite(z_params& zs, Flush flush, Modio::ErrorCode& ec)
{
    ...
    for(;;)
    {
        ...
        case TYPEDO:
        {
            ...
            default:
                return err(Modio::make_error_code(Modio::ZlibError::InvalidBlockType));
            }
            break;
        }
        ...
        case STORED:
        {
            ...
            if(length_ != ((v >> 16) ^ 0xffff))
                return err(Modio::make_error_code(Modio::ZlibError::InvalidStoredLength));
        ...
        case CODELENS:
        {
            while(have_ < nlen_ + ndist_)
            {
                ...
                else
                {
                    ...
                    if(cp->val == 16)
                    {
                        ...
                        bi_.drop(cp->bits);
                        if(have_ == 0)
                            return err(Modio::make_error_code(Modio::ZlibError::InvalidBitLengthRepeat));
                        bi_.read(copy, 2);
                        len = lens_[have_ - 1];
                        copy += 3;
            ...
            if(lens_[256] == 0)
                return err(Modio::make_error_code(Modio::ZlibError::MissingEOB));
        ...
        case MATCH:
        {
            ...
            if(offset_ > r.out.used())
            {
                ...
                auto offset = static_cast<std::uint16_t>(
                    offset_ - r.out.used());
                if(offset > w_.size())
                    return err(Modio::make_error_code(Modio::ZlibError::InvalidDistance));
    ...
    Modio::ErrorCode& ec)
{
    ...
    for (len = 1; len <= 15; len++)
    {
        ...
        if (left < 0)
        {
            ec = Modio::make_error_code(Modio::ZlibError::OverSubscribedLength);
            return;
        }
    }
    if (left > 0 && (type == build::codes || max != 1))
    {
        ec = Modio::make_error_code(Modio::ZlibError::IncompleteLengthSet);
        return;
    }
...
inflate_fast(ranges& r, Modio::ErrorCode& ec)
{
    ...
    dolen:
        ...
        dodist:
            ...
            if(op & 16)
            {
                ...
                if(dist > op)
                {
                    ...
                    if(op > w_.size())
                    {
                        ec = Modio::make_error_code(Modio::ZlibError::InvalidDistance);
                        mode_ = BAD;
                        break;
                    }
            ...
            {
                ec = Modio::make_error_code(Modio::ZlibError::InvalidDistanceCode);
                mode_ = BAD;
                break;
            }
        }
        ...
        {
            ec = Modio::make_error_code(Modio::ZlibError::InvalidLiteralLength);
            mode_ = BAD;
            break;
        }
    }
