...
FModioPagedResult::FModioPagedResult(int32 ResultOffset, int32 PageSize, int32 TotalResultCount, int32 ResultCount)
	: PageIndex(static_cast<int32>(FMath::FloorToInt(double(ResultOffset) / double(PageSize)))),
	  PageSize(PageSize),
	  PageCount(static_cast<int32>(FMath::CeilToInt(double(TotalResultCount) / double(PageSize)))),
