...
void ModioCreateModParamsDetails::DrawCreateMod(IDetailLayoutBuilder& DetailBuilder)
{
	...
	IDetailCategoryBuilder& ModPropertiesCategory = DetailBuilder.EditCategory("Modio Create Mod Param", FText::FromString("Create Mod Properties"));
	ModPropertiesCategory.AddCustomRow(FText::FromString("PathToLogoFile"))
	.NameContent()
	[	
		PathToLogoFileProperty->CreatePropertyNameWidget()
	]
	.ValueContent()
	[
		SNew(SHorizontalBox)
		...
		.AutoWidth()
		[
			SAssignNew(PathToLogoFileEditableTextBox, SEditableTextBox)
		...
		[
			...
			.OnClicked_Lambda([this]()
			{
				...
				WindowManager::Get().OpenFileDialog("Choose your thumbnail:", "PNG Files (*.png)|*.png", false, Thumbnails);
				if (Thumbnails.Num() > 0)
				{
	...
	.NameContent()
	[	
		NameProperty->CreatePropertyNameWidget()
	]
	.ValueContent()
	[
		SNew(SHorizontalBox)
		...
		[
			...
			{
				Target->Name = NameEditableTextBox->GetText().ToString();
			})
		]
	];
	...
	.NameContent()
	[	
		SummaryProperty->CreatePropertyNameWidget()
	]
	.ValueContent()
	...
	[
		...
		.MaxWidth(256.f)
		[
			SAssignNew(SummaryMultiLineEditableTextBox, SMultiLineEditableTextBox)
			.AllowMultiLine(true)
			.AutoWrapText(true)															
