				 ...
				 .AutoHeight()[SNew(SBox).HAlign(
					 HAlign_Center)[SNew(SHyperlinkLaunchURL, TEXT("https://docs.mod.io/unreal/core-plugin/"
										...
										.ToolTipText(LOCTEXT("ModioPluginSettingsDocumentationTooltip",
															 "Opens a page that outlines the configuration settings "
															 "for the mod.io plugin."))]] +
			 SVerticalBox::Slot()
				 .Padding(FMargin(0, 5, 5, 5))
				 .AutoHeight()[SNew(STextBlock)
								   .Text(LOCTEXT(
									   "ModioPluginsSettingsConfigGuid",
									   "If this is your first time setting up the plugin and/or working with mod.io, "
									   "press the button below for and assisted setup process.\n\r"
...
void FModioSettingsDetails::DrawThrobber()
{
	...
	ConfigContentBox->AddSlot()
		...
		.HAlign(HAlign_Center)
		.VAlign(VAlign_Center)[SNew(SCircularThrobber).Radius(50.f)];
}
...
void FModioSettingsDetails::DrawConfigGuide()
{
	...
	ConfigContentBox->AddSlot()
		...
		.VAlign(VAlign_Center)
		.AutoHeight()[SNew(STextBlock)
						  .Text(LOCTEXT("FirstTimePreamble",
										"If you have not made an account with mod.io, do so now here:"))];

	ConfigContentBox->AddSlot()
		.Padding(FMargin(5.f, 5.f, 5.f, 5.f))
		.HAlign(HAlign_Center)
		.VAlign(VAlign_Center)
		.AutoHeight()[SNew(SHyperlinkLaunchURL, TEXT("https://mod.io/login"))
						  .Text(LOCTEXT("ModioPluginLogin", "mod.io Sign up"))
						  .ToolTipText(LOCTEXT("ModioPluginLoginTooltip", "Opens the login/sign up page for mod.io"))];
	...
	ConfigContentBox->AddSlot()
		...
		.VAlign(VAlign_Center)
		.AutoHeight()[SNew(STextBlock)
						  .Text(LOCTEXT("FirstTimeMakeGame", "Next, if you haven't already, you will need to "
															 "make/register your game with mod.io, here:"))];
	...
	ConfigContentBox->AddSlot()
		...
		.AutoHeight()[SNew(SHyperlinkLaunchURL, TEXT("https://mod.io/g/add/"))
						  ...
						  .ToolTipText(
							  LOCTEXT("ModioPluginIntegrateTooltip", "Opens the game integration page for mod.io"))];
	...
	ConfigContentBox->AddSlot()
		...
		.AutoHeight()[SNew(STextBlock)
						  .Text(LOCTEXT(
							  "FirstTimeGetKeys",
							  "Finally, you need to agree to the API Access Terms and get your Game ID and API Key.\r\n"
							  "1. Click the link below, you will find the Agreement Terms, and a list of all your "
							  "games on mod.io\r\n"
	...
	ConfigContentBox->AddSlot()
		...
		.AutoHeight()[SNew(SHyperlinkLaunchURL, TEXT("https://mod.io/me/access"))
						  ...
						  .ToolTipText(LOCTEXT("ModioPluginKeysTooltip",
											   "Opens the API access page for the logged in user on mod.io"))];
	...
	ConfigContentBox->AddSlot()
		...
		.AutoHeight()[SNew(SHorizontalBox) +
					  SHorizontalBox::Slot()
						  ...
						  .HAlign(HAlign_Left)
						  .VAlign(VAlign_Center)
						  .AutoWidth()[SNew(STextBlock).Text(LOCTEXT("GameID", "Game ID:"))] +
					  SHorizontalBox::Slot()
						  .Padding(FMargin(5.f, 5.f, 5.f, 5.f))
						  .HAlign(HAlign_Left)
						  .VAlign(VAlign_Center)
	...
	ConfigContentBox->AddSlot()
		...
		.AutoHeight()[SNew(SHorizontalBox) +
					  SHorizontalBox::Slot()
						  ...
						  .HAlign(HAlign_Left)
						  .VAlign(VAlign_Center)
						  .AutoWidth()[SNew(STextBlock).Text(LOCTEXT("APIKey", "API Key:"))] +
					  SHorizontalBox::Slot()
						  .Padding(FMargin(5.f, 5.f, 5.f, 5.f))
						  .HAlign(HAlign_Left)
						  .VAlign(VAlign_Center)
						  .AutoWidth()[SAssignNew(ApiKeyInput, SEditableTextBox)
										   .MinDesiredWidth(256.f)
...
{
	ConfigWindow = SNew(SWindow)
					   .Title(LOCTEXT("ConfigWindowTitle", "Configure"))
					   .ClientSize(FVector2D(625.f, 350.f))
					   .SupportsMaximize(false)
					   .SupportsMinimize(false)
					   .SizingRule(ESizingRule::FixedSize)[SAssignNew(ConfigContentBox, SVerticalBox)];
