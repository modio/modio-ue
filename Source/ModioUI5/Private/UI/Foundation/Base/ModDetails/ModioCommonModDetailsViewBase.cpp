...
FModioModID UModioCommonModDetailsViewBase::GetModID_Implementation()
{
	if (!DataSource)
	{
		UE_LOG(ModioUI5, Error, TEXT("Unable to get mod ID for mod details '%s': DataSource is invalid"), *GetName());
		return FModioModID();
	}
	
	if (!DataSource->Implements<UModioModInfoUIDetails>())
	{
		UE_LOG(ModioUI5, Error, TEXT("Unable to get mod ID for mod details '%s': DataSource '%s' does not implement UModioModInfoUIDetails"), *GetName(), *DataSource->GetName());
		return FModioModID();
	}
...
FModioModInfo UModioCommonModDetailsViewBase::GetFullModInfo_Implementation()
{
	if (!DataSource)
	{
		UE_LOG(ModioUI5, Error, TEXT("Unable to get mod info for mod details '%s': DataSource is invalid"), *GetName());
		return FModioModInfo();
	}
	
	if (!DataSource->Implements<UModioModInfoUIDetails>())
	{
		UE_LOG(ModioUI5, Error, TEXT("Unable to get mod info for mod details '%s': DataSource '%s' does not implement UModioModInfoUIDetails"), *GetName(), *DataSource->GetName());
		return FModioModInfo();
	}
